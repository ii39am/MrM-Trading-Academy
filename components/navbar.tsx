...
export function Navbar({ locale, initialUser }: { locale: Locale; initialUser: User | null }) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-violet-200/[.08] bg-[#070511]/80 shadow-[0_10px_45px_rgba(3,2,8,.2)] backdrop-blur-2xl supports-[backdrop-filter]:bg-[#070511]/68">
      <nav className="container-pad flex h-20 items-center justify-between gap-5" aria-label={ar ? "التنقل الرئيسي" : "Main navigation"}>
        ...
        <div className="hidden items-center gap-1 lg:flex"><LanguageSwitcher locale={locale} />{actionLinks}</div>
      ...
      {open && <div id="mobile-navigation" className="border-t border-violet-200/[.08] bg-[#090615]/98 px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 shadow-2xl lg:hidden"><div className="mx-auto max-w-7xl"><div className="mb-4"><LanguageSwitcher locale={locale} /></div>{links.map(([label, href]) => <Link key={href} href={href} aria-current={pathname === href ? "page" : undefined} className={cn("block rounded-xl px-4 py-3.5 text-base transition", pathname === href ? "bg-violet-500/12 text-violet-100" : "text-violet-100/62 hover:bg-white/[.04] hover:text-white")}>{label}</Link>)}<div className="mt-4 grid gap-2 border-t border-violet-200/[.08] pt-4">{actionLinks}</div></div></div>}
    </header>
  );
}
