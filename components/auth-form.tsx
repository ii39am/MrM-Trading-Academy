...
export function AuthForm({mode,locale}:{mode:"login"|"register";locale:Locale}){
 ...
 return <div className="w-full max-w-md">
  ...
  <form ref={formRef} onSubmit={submit} className="mt-8 space-y-5" noValidate>
   ...
   <label className="block text-xs text-white/45"><span className="flex justify-between">{ar?"كلمة المرور":"Password"}{mode==="login"&&<Link href="/forgot-password" className="text-blue-400">{ar?"نسيت كلمة المرور؟":"Forgot password?"}</Link>}</span><span className="relative mt-2 block"><input name="password" type={show?"text":"password"} required minLength={10} autoComplete={mode==="login"?"current-password":"new-password"} className="input pe-12"/><button type="button" onClick={()=>setShow(!show)} className="absolute end-4 top-3.5 text-white/30" aria-label={show?"Hide password":"Show password"}>{show?<EyeOff className="h-4 w-4"/>:<Eye className="h-4 w-4"/>}</button></span></label>
   {mode==="register"&&<><label className="block text-xs text-white/45">{ar?"تأكيد كلمة المرور":"Confirm password"}<input name="passwordConfirmation" type="password" required minLength={10} autoComplete="new-password" className="input mt-2"/></label><label className="flex items-start gap-3 text-xs text-white/55"><input name="termsAccepted" type="checkbox" required className="mt-0.5 h-4 w-4 accent-blue-600"/><span>{ar?"أوافق على الشروط وإخلاء مسؤولية المخاطر.":"I accept the Terms and acknowledge the Risk Disclosure."}</span></label></>}
  ...
  <p className="mt-7 text-center text-sm text-white/40"><Link className="font-medium text-blue-400" href={mode==="login"?"/register":"/login"}>{ar?(mode==="login"?"إنشاء حساب جديد":"لديك حساب؟ سجّل الدخول"):(mode==="login"?"Create account":"Already verified? Sign in")}</Link></p>
 </div>;
}
