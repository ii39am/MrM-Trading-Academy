...
const mapStatus=(status:string):"PENDING"|"PAID"|"FAILED"|"EXPIRED"|"CANCELLED"|"REFUNDED"=>{if(["finished","confirmed"].includes(status))return "PAID";if(status==="failed")return "FAILED";if(status==="expired")return "EXPIRED";if(status==="cancelled")return "CANCELLED";if(status==="refunded")return "REFUNDED";return "PENDING"};
export class NowPaymentsProvider implements PaymentProvider{
 ...
 async createCheckout(input:CheckoutInput):Promise<CheckoutSession>{
  const response=await fetch("https://api.nowpayments.io/v1/payment",{method:"POST",headers:{"x-api-key":this.apiKey,"Content-Type":"application/json"},body:JSON.stringify({price_amount:(input.amountCents/100).toFixed(2),price_currency:"usd",pay_currency:"usdttrc20",order_id:input.purchaseId,order_description:`Mr.ME Trading Academy purchase ${input.purchaseId}`,ipn_callback_url:this.callbackUrl,success_url:`${this.appUrl}/dashboard/purchases/${input.purchaseId}`,cancel_url:`${this.appUrl}/courses`}),signal:AbortSignal.timeout(10000)});
