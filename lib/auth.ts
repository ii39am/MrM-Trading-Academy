...
export async function createSession(user:User){
 ...
 await db.session.create({data:{id:hashSessionId(id),userId:user.id,expiresAt}});
...
export async function getSessionUser():Promise<User|null>{
 ...
 const [user,session]=await Promise.all([
  ...
  db.session.findUnique({where:{id:hashSessionId(payload.jti)}})
 ]);
...
export async function revokeCurrentSession(){
 const payload=await readPayload();
 if(payload?.jti)await db.session.updateMany({where:{id:hashSessionId(payload.jti),revokedAt:null},data:{revokedAt:new Date()}});
}
