...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const {id}=await params,course=await db.course.findUnique({where:{id},select:{id:true,telegramAccessUrl:true}});if(!course)return errorResponse("NOT_FOUND","Course not found",404);
 if(parsed.data.published&&!course.telegramAccessUrl)return errorResponse("ACCESS_REQUIRED","Telegram access must be configured before publication",409);
