...
export async function POST(request:Request){
 ...
 const accountRate=await enforceRateLimit(`login-email:${email}`,8,15*60_000);if(!accountRate.allowed)return rateLimited(accountRate.retryAfter);
 ...
 await createSession(result.user);return Response.json({user:result.user});
}
