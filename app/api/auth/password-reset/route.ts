...
export async function POST(request:Request){
 ...
 const emailRate=await enforceRateLimit(identifierKey("password-reset-email",email),3,60*60_000);if(!emailRate.allowed)return rateLimited(emailRate.retryAfter);
 try{
