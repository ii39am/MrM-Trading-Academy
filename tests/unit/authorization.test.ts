...
describe("course authorization",()=>{
 ...
 it("denies absent enrollment",async()=>{findFirst.mockResolvedValue(null);const {requireEnrollment}=await import("@/lib/authorization");await expect(requireEnrollment("u1","course")).resolves.toBeNull()});
});
