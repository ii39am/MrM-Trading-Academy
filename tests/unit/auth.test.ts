...
describe("session verification",()=>{
 it("stores only a one-way session identifier",async()=>{const {hashSessionId}=await import("@/lib/auth");expect(hashSessionId("session-id")).toMatch(/^[a-f0-9]{64}$/);expect(hashSessionId("session-id")).not.toBe("session-id")});
